var picture = (function($) {
	...
	picture.genHtml = function(id) {
		...
		if(this.arrImg.length%2==0){

			for (var i = 0; i < this.arrImg.length; i++) {
				html += "<div class='imgBlocks'  onMouseover='picture.mYMouseOver(this.id)' onmouseout='picture.myMouserOut(this.id)'><div class='imgTop'><img src='" + this.arrImg[i].img + "' id='img" + (i + 1 + 'action') + "'/></div><div class='imgTitle'  onclick='picture.myClick(this.id+\"action\")' id='img" + (i + 1) + "' >" + this.arrImg[i].title + "</div></div>";
			}
